...
# TODO ignoring this because this should only ever be used during ROS2 build,
# decide if we ever need to actually import the package proper
# Maybe if we call this script after setuptools I can just import it normally?
import sys
sys.path.append(os.path.join(get_canros_dir(), "canros"))
import uavcan_msgs
import constants
del sys
...
def print_type(typ, f=None):
	if typ.category == typ.CATEGORY_PRIMITIVE:
		...
		print(uavcan_msgs.ros_name(typ.full_name), end='', file=f)
...
def print_id(f=None):
	...
	print_field(uavcan.dsdl.parser.Field(id_tag_type, constants.uavcan_id_field_name), f=f)
...
def print_union(fields, f=None):
	...
	for i in range(0, len(fields)):
		print_constant(uavcan.dsdl.parser.Constant(
			union_tag_type,
			constants.union_const_prefix + fields[i].name.upper(),
			str(i), i
		), f=f)
...
uint8[] name
""").substitute(
			NodeStatus=uavcan_msgs.ros_name("uavcan.protocol.NodeStatus"),
			SoftwareVersion=uavcan_msgs.ros_name("uavcan.protocol.SoftwareVersion"),
			HardwareVersion=uavcan_msgs.ros_name("uavcan.protocol.HardwareVersion")
		),
		sep='', end='', file=f
	)
...
def main():
	...
	for _, typ in uavcan.TYPENAMES.items():
		if typ.kind == typ.KIND_MESSAGE:
			file_path = os.path.join(ros_msg_dir, uavcan_msgs.ros_name(typ.full_name) + ".msg")
		else:
			file_path = os.path.join(ros_srv_dir, uavcan_msgs.ros_name(typ.full_name) + ".srv")
		...
		with open(file_path, 'w+') as f:
			...
			if typ.kind == typ.KIND_MESSAGE:
				...
				if uavcan_msgs.ros_has_id_field(typ.full_name):
					print_id(f=f)
