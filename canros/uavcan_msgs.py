...
# TODO figure out why ROS2 installation screws up imports like this
try:
	import constants
except ModuleNotFoundError:
	from canros import constants
...
def ros_topic(name, typ):
	return constants.ros_topic_prefix + "/" + typ + "/" + name.replace(".", "/")

# Returns the ROS name for a given full UAVCAN name.
def ros_name(name):
	# TODO come up with an actual solution for mapping names for ROS2
...
def ros_has_id_field(name):
	return name in constants.msgs_with_id
	...
	if uavcan_type.category == uavcan_type.CATEGORY_COMPOUND:
		...
		for i, field in enumerate(fields):
			...
			try:
				...
				if union:
					setattr(ros_msg, constants.union_tag_field_name, i)
		return ros_msg
...
class Message(_Base):
	...
	'''
	def Publisher(self, ros_node: rclpy.node.Node, queue_size, **kw):
		return ros_node.create_publisher(self.Type, self.Topic, queue_size, **kw)
	...
	'''
	def Subscriber(self, ros_node: rclpy.node.Node, callback, queue_size, **kw):
		return ros_node.create_subscription(self.Type, self.Topic, callback, queue_size, **kw)
...
class Service(_Base):
	...
	'''
	def Client(self, ros_node: rclpy.node.Node, **kw):
		return ros_node.create_client(self.Type, self.Request_Topic, **kw)
	...
	'''
	def Service(self, ros_node: rclpy.node.Node, handler, **kw):
		return ros_node.make_service(self.Type, self.Response_Topic, handler, **kw)
